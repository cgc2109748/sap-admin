...
export const queryProductByType = createAsyncThunk('products/queryProductByType', async (productData, thunkAPI) => {
  try {
    const token = thunkAPI.getState().auth.user.token;
    return await productService.queryProductByType(productData, token);
  } catch (error) {
    const message =
      (error.response && error.response.data && error.response.data.message) || error.message || error.toString();
    return thunkAPI.rejectWithValue(message);
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(queryProductByType.fulfilled, (state, action) => {
        state.isLoading = false;
        state.isSuccess = true;
        state.products = state.products.filter((product) => product.type !== action.payload.type);
      })
